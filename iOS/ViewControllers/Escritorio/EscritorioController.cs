...
{
    public partial class EscritorioController : UITableViewController
    {
        const string IdentificadorCeldaHeader = "Header";
        const string IdentificadorCeldaPost = "Post";
        const string IdentificadorCeldaImagenPost = "PostImage";
        const string IndentificadorCeldaImagenPerfil = "NotificacionImagenPerfil";
        const string IdentificadorCeldaImagenFondo = "NotificacionImagenFondo";
        ...
        UIImage currentProfileImage = new UIImage();
        UIImage currentPostImage = new UIImage();
        UIImageView ImagenPerfil;

        public List<ItemsMenu> tableItems = new List<ItemsMenu>();

        List<UsuarioModel> UsuariosLikes = new List<UsuarioModel>();
        ...
        async void GetPost(NSIndexPath indexPath)
        {
            ...
            foreach (PostModel post in MenuHelper.AllPost)
            {
        ...
        {
            var headerCell = (EscritorioHeaderCell)tableView.DequeueReusableCell(IdentificadorCeldaHeader);
            headerCell.UpdateCell();
            headerCell.EventosHeaderDelegate = this;
            this.ImagenPerfil = headerCell.getImagenPerfil();
            StyleHelper.Style(headerCell.Layer);
        ...
        public override UITableViewCell GetCell(UITableView tableView, Foundation.NSIndexPath indexPath)
        {
            if (isShowInformation)
            {
                ...
                if (current.Publicacion_Tipo == 0 || current.Publicacion_Tipo == null)
                {
                    if ((current.Publicacion_Imagen_Ruta != null && current.Publicacion_Imagen_Ruta != "") && (current.Publicacion_Imagen != null && current.Publicacion_Imagen != ""))
                    {
                        var currentImagePostCell = (ComentariosBodyImageCell)tableView.DequeueReusableCell(IdentificadorCeldaImagenPost, indexPath);
                        currentImagePostCell.UpdateCell(current, this.FotosId, this.FotosPerfilId);
                        currentImagePostCell.EventosComentariosDelegate = this;
                        return currentImagePostCell;
                    }
                    else
                    {
                        ...
                        currentPostCell.EventosComentariosBodyDel = this;
                        return currentPostCell;
                    }
                }
                else if (current.Publicacion_Tipo == 1)
                {
                    var currentImagePostCell = (CeldaImagenPerfil)tableView.DequeueReusableCell(IdentificadorCeldaImagenPost, indexPath);
                    currentImagePostCell.UpdateCell(current);
                    currentImagePostCell.EventosComentariosDelegate = this;
                    return currentImagePostCell;
                }
                else
                {
                    var currentImagePostCell = (NotificacionImagenFondo)tableView.DequeueReusableCell(IdentificadorCeldaImagenPost, indexPath);
                    currentImagePostCell.UpdateCell(current);
                    currentImagePostCell.EventosComentariosDelegate = this;
                    return currentImagePostCell;
                }
        ...
        {
            if (segue.Identifier == "toShowPostView")
            {
                var postCommentView = (PublicarPostViewController)segue.DestinationViewController;
                postCommentView.PostPublicadoDelegate = this;
            }
            else if (segue.Identifier == "comentar")
            {
                var CommentView = (comentarTableView)segue.DestinationViewController;
                CommentView.currentPost = CurrentPost;
            }
            else if (segue.Identifier == "toShowImageFromPost")
            {
                var ImageView = (DetailCommentImage)segue.DestinationViewController;
                ImageView.ImagenPost = (UIImageView)sender;
            }

            else if (segue.Identifier == "DetallarPerfil")
            {
            ...
            else if (segue.Identifier == "VistaLikes")
            {
                var LikesView = (VistaLikes)segue.DestinationViewController;
    ...
    partial class EscritorioController : PostPublicadoDel
    {
        public async void PostPublicado()
        {
            BTProgressHUD.Show("");
            await MenuHelper.GetMuroPosts(ContPag, 5);
            allPosts = MenuHelper.AllPost;
    ...
    partial class EscritorioController : EventosComentarios
    {
        public void MostrarImagenEnGrande(UIImageView imgPublicacion)
        {
            PerformSegue("toShowImageFromPost", imgPublicacion);
        }
        public void ComentarPost(PostModel PostLocal)
        {
            CurrentPost = PostLocal;
            BTProgressHUD.Show("");
            this.PerformSegue("comentar", null);
        }
        ...
        public void LikePresionado(List<UsuarioModel> UsuariosLikes)
        {
            this.UsuariosLikes = UsuariosLikes;
            this.PerformSegue("VistaLikes", null);
        }
    ...
    partial class EscritorioController : EventosComentariosBody
    {
        public void ComentarPosts(PostModel PostLocal)
        {
            CurrentPost = PostLocal;
            BTProgressHUD.Show("");
            this.PerformSegue("comentar", null);
        ...
        public void InfoUserPosts(UsuarioModel listaUser)
        {
            ListUser = listaUser;
            this.PerformSegue("DetallarPerfil", null);
        ...
        public void LikeImagenPresionado(List<UsuarioModel> UsuariosLikes)
        {
            this.UsuariosLikes = UsuariosLikes;
            this.PerformSegue("VistaLikes", null);
        }
    ...
    partial class EscritorioController : EventosHeader
    {
