using System; using UIKit; using WorklabsMx.iOS.Helpers; using WorklabsMx.Models; using System.Collections.Generic; using WorklabsMx.Controllers; using BigTed; using WorklabsMx.Helpers; using System.Threading.Tasks; using Foundation; using SWRevealViewControllerBinding;  namespace WorklabsMx.iOS {     public partial class EscritorioController : UITableViewController
    {
        ...
        const string IdentificadorCeldaPost = "Post";         const string IdentificadorCeldaImagenPost = "PostImage";         const string IndentificadorCeldaImagenPerfil = "NotificacionImagenPerfil";         const string IdentificadorCeldaImagenFondo = "NotificacionImagenFondo";
...
         List<PostModel> allPosts = new List<PostModel>();                UsuarioModel ListUser = new UsuarioModel();          PostModel CurrentPost = new PostModel();         UIImage currentProfileImage = new UIImage();         UIImage currentPostImage = new UIImage();         UIImageView ImagenPerfil;          public List<ItemsMenu> tableItems = new List<ItemsMenu>();          List<UsuarioModel> UsuariosLikes = new List<UsuarioModel>();          float withImage;         public int Contador;          int ContPag = 0;          List<string> FotosId;         List<string> FotosPerfilId; 
        public EscritorioController(IntPtr handle) : base(handle)         {
        }

        public override async void ViewDidLoad()
        {
            base.ViewDidLoad();             FotosId = new List<string>();             await MenuHelper.FillTable();             Contador = 5;             RefreshControl = new UIRefreshControl();             RefreshControl.AddTarget(HandleValueChanged, UIControlEvent.ValueChanged);             await MenuHelper.GetMuroPosts(ContPag, 5);             allPosts = MenuHelper.AllPost;             TableView.ReloadData();             this.TableView.BeginUpdates();             this.TableView.EndUpdates();              withImage = float.Parse(UIScreen.MainScreen.Bounds.Width.ToString());             UIApplication.SharedApplication.StatusBarStyle = UIStatusBarStyle.LightContent;             var Tap = new UITapGestureRecognizer(this.Tapped);             this.View.AddGestureRecognizer(Tap);         }
        ...
        public override UITableViewCell GetCell(UITableView tableView, Foundation.NSIndexPath indexPath)
        {
            ...
            {                                  if (indexPath.Row == allPosts.Count - 1)                 {                     ContPag++;                     this.GetPost(indexPath);                 }
                var current = allPosts[indexPath.Row];                 if(current.Publicacion_Tipo == 0 || current.Publicacion_Tipo == null)                 {                     if ((current.Publicacion_Imagen_Ruta != null && current.Publicacion_Imagen_Ruta != "") && (current.Publicacion_Imagen != null && current.Publicacion_Imagen != ""))                     {                         var currentImagePostCell = (ComentariosBodyImageCell)tableView.DequeueReusableCell(IdentificadorCeldaImagenPost, indexPath);                         currentImagePostCell.UpdateCell(current, this.FotosId, this.FotosPerfilId);                         currentImagePostCell.EventosComentariosDelegate = this;                         return currentImagePostCell;                     }                     else                     {                         var currentPostCell = (ComentariosBodyCell)tableView.DequeueReusableCell(IdentificadorCeldaPost, indexPath);                         currentPostCell.UpdateCell(current);                         currentPostCell.EventosComentariosBodyDel = this;                         return currentPostCell;                     }                 }                 else if(current.Publicacion_Tipo == 1)                 {                     var currentImagePostCell = (CeldaImagenPerfil)tableView.DequeueReusableCell(IdentificadorCeldaImagenPost, indexPath);                     currentImagePostCell.UpdateCell(current);                     currentImagePostCell.EventosComentariosDelegate = this;                     return currentImagePostCell;                 }                 else                 {                     var currentImagePostCell = (NotificacionImagenFondo)tableView.DequeueReusableCell(IdentificadorCeldaImagenPost, indexPath);                     currentImagePostCell.UpdateCell(current);                     currentImagePostCell.EventosComentariosDelegate = this;                     return currentImagePostCell;                 }               
            }
            else
            {
        ...
        }          private void Tapped(UITapGestureRecognizer Recognizer)         {             this.View.EndEditing(true);         }     }      partial class EscritorioController: PostPublicadoDel     {         public async void PostPublicado()         {             BTProgressHUD.Show("");             await MenuHelper.GetMuroPosts(ContPag, 5);             allPosts = MenuHelper.AllPost;             TableView.ReloadData();             this.TableView.BeginUpdates();             this.TableView.EndUpdates();             BTProgressHUD.Dismiss();             this.Contador = 5;             this.UpdateRows();         }     }       partial class EscritorioController: EventosComentarios     {         public void MostrarImagenEnGrande(UIImageView imgPublicacion)         {             PerformSegue("toShowImageFromPost", imgPublicacion);         }         public void ComentarPost(PostModel PostLocal)         {             CurrentPost = PostLocal;
            BTProgressHUD.Show("");             this.PerformSegue("comentar", null);         }         public void InfoUserPost(UsuarioModel listaUser)         {             ListUser = listaUser;
            this.PerformSegue("DetallarPerfil", null);         }          public void EnviarActions(UIAlertController actionSheetAlert)         {             UIPopoverPresentationController presentationPopover = actionSheetAlert.PopoverPresentationController;             if (presentationPopover != null)             {                 presentationPopover.SourceView = this.View;                 presentationPopover.PermittedArrowDirections = UIPopoverArrowDirection.Up;             }             this.PresentViewController(actionSheetAlert, true, null);         }          public async void ActualizaTabla()         {             await MenuHelper.GetMuroPosts(ContPag, 5);             allPosts = MenuHelper.AllPost;             TableView.ReloadData();             this.TableView.BeginUpdates();             this.TableView.EndUpdates();             this.Contador = 5;             this.UpdateRows();         }          public void UpdateRows()         {             if(this.Contador > 0)             {                 Contador = Contador - 1;                 System.Threading.Thread.Sleep(500);                 this.TableView.ReloadData();                 this.TableView.BeginUpdates();                 this.TableView.EndUpdates();             }          }          public void LikePresionado(List<UsuarioModel> UsuariosLikes)         {             this.UsuariosLikes = UsuariosLikes;             this.PerformSegue("VistaLikes", null);         }          public void ImagenPublicada(List<string> FotosId)         {             this.FotosId = FotosId;         }          public void ImagenPerfilPublicada(List<string> FotosPerfilId)         {             this.FotosPerfilId = FotosPerfilId;         }     }      partial class EscritorioController : EventosComentariosBody     {         public void ComentarPosts(PostModel PostLocal)         {             CurrentPost = PostLocal;             BTProgressHUD.Show("");             this.PerformSegue("comentar", null);             BTProgressHUD.Dismiss();         }         public void InfoUserPosts(UsuarioModel listaUser)         {             ListUser = listaUser;             this.PerformSegue("DetallarPerfil", null);                     }          public void EnviarAction(UIAlertController actionSheetAlert)         {             UIPopoverPresentationController presentationPopover = actionSheetAlert.PopoverPresentationController;             if (presentationPopover != null)             {                 presentationPopover.SourceView = this.View;                 presentationPopover.PermittedArrowDirections = UIPopoverArrowDirection.Up;             }             this.PresentViewController(actionSheetAlert, true, null);         }          public async void ActualizarTabla()         {             await MenuHelper.GetMuroPosts(ContPag, 5);             allPosts = MenuHelper.AllPost;             TableView.ReloadData();             this.TableView.BeginUpdates();             this.TableView.EndUpdates();         }          public void UpdateRowsCom()         {             if (this.Contador > 0)             {                 Contador = Contador - 1;                 System.Threading.Thread.Sleep(500);                 this.TableView.ReloadData();                 this.TableView.BeginUpdates();                 this.TableView.EndUpdates();             }          }          public void LikeImagenPresionado(List<UsuarioModel> UsuariosLikes)         {             this.UsuariosLikes = UsuariosLikes;             this.PerformSegue("VistaLikes", null);         }       }      partial class EscritorioController : EventosHeader     {         public async void Buscando(string Texto)         {             await Task.Run(() =>             {
