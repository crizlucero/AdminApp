using System; using UIKit; using CoreGraphics; using WorklabsMx.iOS.Helpers; using WorklabsMx.Models; using WorklabsMx.iOS.Styles; using System.Collections.Generic; using PerpetualEngine.Storage; using System.Threading.Tasks; using WorklabsMx.iOS.ViewElements; using WorklabsMx.Helpers; using SVProgressHUDBinding; using WorklabsMx.Controllers;   namespace WorklabsMx.iOS {     public partial class EscritorioController : UITableViewController //UIViewController
    {
        ...
        {
            base.ViewWillAppear(animated);             posts = new Controllers.EscritorioController().GetMuroPosts(currentPage);
        }

        public override UIView GetViewForHeader(UITableView tableView, nint section)
        {             var storageLocal = PerpetualEngine.Storage.SimpleStorage.EditGroup("Login");             var miembro = new MiembrosController().GetMemberName(storageLocal.Get("Usuario_Id"), storageLocal.Get("Usuario_Tipo"));
...
 
        ...
        public override void WillDisplay(UITableView tableView, UITableViewCell cell, Foundation.NSIndexPath indexPath)
        {
            ...
            {
                currentPage += 5;                 posts = new Controllers.EscritorioController().GetMuroPosts(currentPage);
                this.TableView.ReloadData();                 this.TableView.ScrollToRow(indexPath, UITableViewScrollPosition.Bottom, false);
            }
        }           public void WillDisplayCell (UICollectionView collectionView, UICollectionViewCell cell, Foundation.NSIndexPath indexPath)         {             var currentCell = cell as DataItemCollectionViewCell;             if (currentCell == null)                 throw new Exception ("Expected to display a DataItemCollectionViewCell");              var item = dataItems [indexPath.Section];              // Configure the cell.             cellComposer.ComposeCell (currentCell, item);         } 
