    ...
    public class InfoEmpresaCard
    {
        public InfoEmpresaCard(EmpresaModel empresa, UIView View, int initialPosition = 0)
        {
            using (UIView headerView = new UIView(new CGRect(0, initialPosition, UIScreen.MainScreen.Bounds.Width, 100)) { BackgroundColor = UIColor.White })
            {
                ...
                headerView.AddSubview(new STLLabel(empresa.Empresa_Nombre, 10, 22));
                using (UIButton lblMail = new STLButton(empresa.Empresa_Correo_Electronico)
                {
                ...
                })
                {
                    ...
                    {
                        UIApplication.SharedApplication.OpenUrl(new NSUrl("mailto:" + empresa.Empresa_Correo_Electronico));
                    };
            ...
            {
                scrollView.AddSubview(new STLImageView(50, empresa.Empresa_Logotipo)
                {
                ...
                scrollView.AddSubview(new STLLabel(empresa.Territorio.Municipio + ", " + empresa.Territorio.Estado + ", "
                                                   + empresa.Empresa_Calle + " #" + empresa.Empresa_Numero_Exterior +
                                                    (!string.IsNullOrEmpty(empresa.Empresa_Numero_Interior) ? " int. " + empresa.Empresa_Numero_Interior : "") +
                ...
                scrollView.AddSubview(new STLLabel(empresa.Empresa_Telefono, 400) { Frame = new CGRect(30, 400, UIScreen.MainScreen.Bounds.Width, 30) });

                scrollView.AddSubview(new STLImageLabel(scrollView, "RFC", 430, "ic_label"));
                scrollView.AddSubview(new STLLabel(empresa.Empresa_Rfc, 460) { Frame = new CGRect(30, 460, UIScreen.MainScreen.Bounds.Width, 30) });

                scrollView.AddSubview(new STLImageLabel(scrollView, "Razón Social", 490, "ic_label"));
                scrollView.AddSubview(new STLLabel(empresa.Empresa_Razon_Social, 520) { Frame = new CGRect(30, 520, UIScreen.MainScreen.Bounds.Width, 30) });

                scrollView.AddSubview(new STLImageLabel(scrollView, "Página Web", 550, "ic_public"));
                scrollView.AddSubview(new STLLabel(empresa.Empresa_Pagina_Web, 580) { Frame = new CGRect(30, 580, UIScreen.MainScreen.Bounds.Width, 30) });

                scrollView.AddSubview(new STLImageLabel(scrollView, "Redes Sociales", 610, "ic_share"));
                scrollView.AddSubview(new STLImageLabel(scrollView, empresa.Empresa_Red_Social_1, 640, "ic_facebook"));
                scrollView.AddSubview(new STLImageLabel(scrollView, empresa.Empresa_Red_Social_2, 670, "ic_twitter"));
                scrollView.AddSubview(new STLImageLabel(scrollView, empresa.Empresa_Red_Social_3, 700, "ic_instagram"));
